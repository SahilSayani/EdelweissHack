...
export default function Tsp() {
  const tsp_content = {
    ...
    list: [
      {
        title: "Analyze the trends",
      },
      {
        title: "Get detailed insight",
      },
      {
          ...
          {tsp_content?.list &&
            tsp_content.list.map((item, i) => (
              <li
                ...
                key={i}
              >
                <div className="flex h-12 w-12 items-center justify-center rounded bg-green-100 text-lg font-bold text-green-600 ">
                  {i + 1}
                </div>
                <h3
